...
import { resolveCallerPlannerIdentity, resolveLogisticsCapability, assignPassenger, normalizePlannerLogisticsError, type ResolvedLogisticsCapability } from '@/lib/plannerLogistics';
import type { SupabaseClient } from '@supabase/supabase-js';

/**
 * Feature 013 — Ground Transport: manage-only "assign" endpoint. Calls
 * Planner's own `assign_or_board_passenger` RPC (never a hand-written
 * insert, plan.md). Portal's own authorization here — not the RPC, which has
 * no internal auth check of its own — is the actual authorization boundary
...
async function resolveAuthorizedContext(eventId: string): Promise<NextResponse | AuthorizedContext> {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('ground-transport/assignments/assign: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('ground-transport/assignments/assign: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('ground-transport/assignments/assign: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  if (canAdminister) {
    ...
    try {
      ...
      console.error('ground-transport/assignments/assign: capability resolution failed', err);
      return NextResponse.json({ ok: true, status: 'backend_error' });
    }
  }
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!Number.isInteger(vehicleId) || vehicleId <= 0) {
    return NextResponse.json({ error: 'invalid_request', message: 'vehicleId is required.' }, { status: 400 });
  }
  ...
  if ('boarded' in body && typeof body.boarded !== 'boolean') {
    return NextResponse.json({ error: 'invalid_request', message: 'boarded must be a boolean.' }, { status: 400 });
  }

  try {
    await assignPassenger(plannerEventId, vehicleId, passengerId, typeof body.boarded === 'boolean' ? body.boarded : true);
