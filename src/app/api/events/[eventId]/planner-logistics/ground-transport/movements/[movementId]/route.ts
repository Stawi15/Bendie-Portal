...
/**
 * Feature 013 — Ground Transport: single-movement route (manage-only field
 * edit on PATCH; manage-only delete on DELETE, blocked while vehicles
 * reference it). Same authorization + item-scope pattern as every prior
 * planner-logistics item route.
 */
...
async function resolveAuthorizedContext(eventId: string): Promise<NextResponse | AuthorizedContext> {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('ground-transport/movements/[movementId]: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('ground-transport/movements/[movementId]: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('ground-transport/movements/[movementId]: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  if (canAdminister) {
    ...
    try {
      ...
      console.error('ground-transport/movements/[movementId]: capability resolution failed', err);
      return NextResponse.json({ ok: true, status: 'backend_error' });
    }
  }
...
function parseMovementId(raw: string): number | null {
  const id = Number(raw);
  return Number.isInteger(id) && id > 0 ? id : null;
}

const PATCH_ALLOWED_FIELDS = new Set(['movementName', 'route', 'movementDate', 'pickupTime', 'notes']);

export async function PATCH(request: NextRequest, { params }: { params: Promise<{ eventId: string; movementId: string }> }) {
  const { eventId, movementId: rawMovementId } = await params;
  ...
  const movementId = parseMovementId(rawMovementId);
  if (!movementId) return NextResponse.json({ error: 'logistics_record_not_found' }, { status: 404 });

  let existing;
  try {
    existing = await getMovement(plannerEventId, movementId);
  } catch (err) {
    console.error('ground-transport/movements/[movementId]: PATCH pre-read failed', err);
  ...
  try {
    ...
    if ('movementName' in body) patch.movementName = body.movementName as string;
    if ('route' in body) patch.route = body.route as string;
    if ('movementDate' in body) patch.movementDate = body.movementDate as string;
    ...
    const movement = await updateMovement(plannerEventId, movementId, patch);
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ eventId: string; movementId: string }> }) {
  const { eventId, movementId: rawMovementId } = await params;
  const context = await resolveAuthorizedContext(eventId);
  if (context instanceof NextResponse) return context;
  ...
  try {
    await deleteMovement(plannerEventId, movementId);
