...
import { resolveCallerPlannerIdentity, resolveLogisticsCapability } from '@/lib/plannerLogistics';

/**
 * Feature 012 — lightweight, dedicated capability-only endpoint for
 * permission-aware Logistics-tab visibility, mirroring
 * `planner-people/capability/route.ts` exactly (same locked-decision
 * composition: `canAdministerPlannerPermissions` first, `can_view_logistics`
 * fallback for an ordinary Planner identity).
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-logistics/capability: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-logistics/capability: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-logistics/capability: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  try {
    const capability = await resolveLogisticsCapability(activeLink.planner_event_id as number, plannerProfileId);
    return NextResponse.json({ ok: true, capability });
  } catch (err) {
    console.error('planner-logistics/capability: capability resolution failed', err);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
}
