...
/**
 * Feature 012 — Hotels: single-record route (manage-only field edit on
 * PATCH; manage-only delete on DELETE). Same authorization + item-scope
 * pattern as `../flights/[recordId]/route.ts`.
 */
...
async function resolveAuthorizedContext(eventId: string): Promise<NextResponse | AuthorizedContext> {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-logistics/hotels/[bookingId]: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-logistics/hotels/[bookingId]: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-logistics/hotels/[bookingId]: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  if (canAdminister) {
    ...
    try {
      ...
      console.error('planner-logistics/hotels/[bookingId]: capability resolution failed', err);
      return NextResponse.json({ ok: true, status: 'backend_error' });
    }
  }
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ eventId: string; bookingId: string }> }) {
  const { eventId, bookingId: rawBookingId } = await params;
  ...
  if (!bookingId) return NextResponse.json({ error: 'logistics_record_not_found' }, { status: 404 });

  let existing;
  try {
    existing = await getHotelBooking(plannerEventId, bookingId);
  } catch (err) {
    console.error('planner-logistics/hotels/[bookingId]: PATCH pre-read failed', err);
  ...
  try {
    ...
    const booking = await updateHotelBooking(plannerEventId, bookingId, patch);
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ eventId: string; bookingId: string }> }) {
  const { eventId, bookingId: rawBookingId } = await params;
  const context = await resolveAuthorizedContext(eventId);
  if (context instanceof NextResponse) return context;
  ...
  try {
    await deleteHotelBooking(plannerEventId, bookingId);
