...
import { resolveCallerPlannerIdentity, resolveProductionCapability } from '@/lib/plannerProduction';

/**
 * Feature 014 — lightweight, dedicated capability-only endpoint for
 * permission-aware Production-tab visibility, mirroring every prior
 * planner-*'s capability route (`canAdministerPlannerPermissions` first,
 * `can_view_production` fallback — the same shape as Logistics/Ground
 * Transport's `can_view_logistics`).
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-production/capability: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-production/capability: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-production/capability: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  try {
    const capability = await resolveProductionCapability(activeLink.planner_event_id as number, plannerProfileId);
    return NextResponse.json({ ok: true, capability });
  } catch (err) {
    console.error('planner-production/capability: capability resolution failed', err);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
}
