...
/**
 * Feature 008 — dedicated, event-scoped viewer-authority check (research.md
 * R11, added during `/speckit.analyze` to resolve the previously-open
 * button-visibility implementation choice). Answers whether the CURRENT
 * viewer may administer *anyone's* Planner permissions for this event — used
 * to gate the Members-page "Bendie Planner Access" entry point without
 * probing an arbitrary `memberId`. Mirrors `planner-tasks/capability/route.ts`'s
 * precedent: steps 1–6 only, no target-member resolution (step 7).
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-permissions/can-administer: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-permissions/can-administer: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-permissions/can-administer: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  return NextResponse.json({ ok: true, canAdminister });
}
