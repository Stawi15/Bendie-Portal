...
import { resolveCallerPlannerIdentity, resolveVendorCapability } from '@/lib/plannerVendors';

/**
 * Feature 009 — lightweight, dedicated capability-only endpoint for
 * permission-aware Vendors-tab visibility, mirroring
 * `planner-tasks/capability/route.ts` exactly (research.md R9/R10). Never
 * fetches vendor items. Unlike the collection route, this endpoint does NOT
 * turn a `canView: false` result — or a missing Planner identity — into an
 * error: `EventLayout.tsx` needs the capability value itself to decide
 * whether to show the tab.
 *
 * Same steps 1-6 as `../route.ts` (copied inline, same rationale — see that
 * file's doc comment), diverging only after Planner identity resolution: this
 * route resolves capability and returns it directly, with no `canView` gate.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-vendors/capability: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-vendors/capability: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-vendors/capability: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  try {
    const capability = await resolveVendorCapability(activeLink.planner_event_id as number, plannerProfileId);
    return NextResponse.json({ ok: true, capability });
  } catch (err) {
    console.error('planner-vendors/capability: capability resolution failed', err);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
}
