...
import { resolveCallerPlannerIdentity, resolvePeopleCapability } from '@/lib/plannerPeople';

/**
 * Feature 011 — lightweight, dedicated capability-only endpoint for
 * permission-aware People-tab visibility, mirroring
 * `planner-vendors/capability/route.ts`. Never fetches participants.
 *
 * Diverges from every prior module after step 5 (plan.md): `canManage` comes
 * ONLY from `canAdministerPlannerPermissions` (Feature 008's existing
 * Portal-side authority — no `can_view_people`/`can_manage_people` flag
 * exists, and none was invented, per this feature's locked product
 * decision). A Portal admin/org-admin gets full access even without a
 * resolvable Planner identity; everyone else needs a Planner identity and
 * either platform-admin status or an active `event_user_assignments` row
 * (any flags) just to view.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-people/capability: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-people/capability: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-people/capability: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  if (canAdminister) {
    return NextResponse.json({ ok: true, capability: { hasPlannerIdentity: true, canView: true, canManage: true } });
  }
  ...
  try {
    const capability = await resolvePeopleCapability(activeLink.planner_event_id as number, plannerProfileId);
    return NextResponse.json({ ok: true, capability });
  } catch (err) {
    console.error('planner-people/capability: capability resolution failed', err);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
}
