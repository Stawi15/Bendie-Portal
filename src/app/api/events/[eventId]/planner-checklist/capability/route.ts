...
import { resolveCallerPlannerIdentity, resolveChecklistCapability } from '@/lib/plannerChecklist';

/**
 * Feature 010 — lightweight, dedicated capability-only endpoint for
 * permission-aware Checklist-tab visibility, mirroring
 * `planner-vendors/capability/route.ts` exactly (which itself mirrors
 * `planner-tasks/capability/route.ts`). Never fetches checklist items.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!isPlatformAdmin) {
    ...
    if (membershipsError) console.error('planner-checklist/capability: organization_members lookup failed', membershipsError);
  ...
  if (!serviceRoleKey) {
    console.error('planner-checklist/capability: missing SUPABASE_SERVICE_ROLE_KEY');
  ...
  const { data: activeLink, error: linkError } = await portalAdmin
    ...
    .maybeSingle();
  if (linkError) {
    console.error('planner-checklist/capability: event_planner_links lookup failed', linkError);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
  if (!activeLink) {
  ...
  try {
    const capability = await resolveChecklistCapability(activeLink.planner_event_id as number, plannerProfileId);
    return NextResponse.json({ ok: true, capability });
  } catch (err) {
    console.error('planner-checklist/capability: capability resolution failed', err);
    return NextResponse.json({ ok: true, status: 'backend_error' });
  }
}
